...
export default function Home() {
  const { selectedHistorique, newHistorique } = useHistoriqueContext();
  ...
  useEffect(() => {
    // Placeholder for any side effects based on selectedHistorique
    if (selectedHistorique && Object.keys(selectedHistorique).length > 0) {
      // Handle selectedHistorique (if needed)
    } else {
      // Handle case where no historique is selected (if needed)
    }
  }, [selectedHistorique]);

  // Toggle between HistoriqueCard and ResultCard
  ...
  return (
    <>
      ...
      <div className="w-full min-h-screen p-20 flex flex-col justify-between z-10">
        {/* Button to toggle between Historique and Result Card */}
          ...
          <div className="flex gap-4">
            <div className="cursor-pointer bg-neutral-100 h-10 w-10 border-solid border-2 border-gray-400 rounded-lg shadow-lg p-1 overflow-auto z-10 flex justify-center">
              <svg
                ...
                viewBox="0 0 512 512"
                onClick={() => toggleDisplay("historique")} // Toggle to show HistoriqueCard
              >
        ...
        {/* Conditionally render HistoriqueCard or ResultCard */}
        {isHistoriqueVisible && <HistoriqueCard close={closeDisplay} />}
        {isResultCardVisible && <ResultCard />}
